...
interface ShaderBackgroundProps {
  /**
   * Define se o background é fixo na viewport (fixed) ou relativo ao container (absolute)
   * @default 'fixed'
   */
...
const ShaderBackground: React.FC<ShaderBackgroundProps> = ({ position = 'fixed' }) => {
  ...
  // Fragment shader source code - CUSTOMIZADO COM CORES DOURADAS/PRETO
  const fsSource = `
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      if (position === 'absolute') {
        // Para absolute, usar dimensões do parent container
        ...
        // Para fixed, usar dimensões da window
